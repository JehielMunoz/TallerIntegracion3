...
class Busqueda_estudiante extends Controller
{
    public function Autocompletar()
    {
            ...
            $resultado = DB::table('tAlumnos')->where('Nombre', 'ILIKE','%' . $Nombre . '%')->get(['Nombre']); // Tal vez limitar resultado o caracteres minimos idk 
            $rut_t=$resultado[0];
    ...
    public function CargarAlumno(){
      ...
      if(request()->isMethod('post')){
        if(request()->filled('Nombre_Estudiante')){
          ...
          $count = DB::table('tAlumnos')->where('Nombre','=',$Nombre)->count();
          if ($count>=1) {
